...
2. The pretrained `MouseHead`/`DiscreteHead` linear layers' weights are pulled out of the raw
   state dict *as tensors* (not reconstructed as modules) so `policy/sb3_policy.py` can splice
   them into specific rows of SB3's own `action_net` linear layer -- giving the aim (`d_yaw`/
   `d_pitch`) and `jump`/`sneak`/`attack`/`place` action dimensions a BC-informed starting point,
   instead of PPO having to relearn "roughly point at the opponent" and "occasionally jump" from
   scratch. `move_forward`/`strafe` have no BC equivalent to inherit (see `nn/README.md`'s
   "Known limitations": `/nn` never modeled movement) and start randomly initialized, same as
   the value head.
"""
...
def build_fresh_backbone(model_cfg: dict, data_meta: RLDataMeta) -> LoadedBackbone:
    ...
    empty_heads = PretrainedHeads(
        mouse_weight=None, mouse_bias=None, discrete_weight=None, discrete_bias=None
    )
    return LoadedBackbone(
        backbone=backbone,
        trunk_dim=backbone.output_dim,
